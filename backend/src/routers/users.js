...
router.post("/registro", async (req, res) => {
       ...
       // let hash = bcrypt.hashSync(usu_password, salt);
        console.log( usu_id, usu_nombre, usu_apellido,  usu_num_documento, usu_telefono, usu_direccion, usu_email, rol_id, ciud_id, doc_id);
  try {
        ...
        datos = await coneccionbd.open(sql, [usu_nombre, usu_apellido, usu_password, usu_num_documento,usu_telefono, usu_direccion, usu_email, rol_id, ciud_id, doc_id ], true);
...
router.post("/ingresar",async (req, res) => {
  ...
  try {
        ...
        if (result[3] != usu_password) return res.send ("Password erroneo");
...
router.put("/actualizar",async (req, res) => {
  ...
  try {
        ...
        sql = `UPDATE USUARIO SET
              ...
              USU_PASSWORD = '${usu_password}', USU_NUM_DOCUMENTO = '${usu_num_documento}', 
